export async function handle(state, action) {
    ...
    if (input.function === "register") {
        ...
        if(contractState && contractState.locked && contractState.locked.length){
            ...
            ContractAssert(lockedObj.vaultAddress==caller, `Caller must be the vault in the locked NFT`);
            const nftIdOnOtherNetwork = input.nftIdOnOtherNetwork;
            if (!nftIdOnOtherNetwork) throw new ContractError("nftIdOnOtherNetwork is undefined");
